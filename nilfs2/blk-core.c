		...
		if (rw & WRITE) {
			count_vm_events(PGPGOUT, count);
                ...
                if(bio->bi_iter.bi_sector!=NULL){    
                    cq.bk_buf[cq.idx] = (unsigned long long) bio->bi_iter.bi_sector;
			
                    do_gettimeofday(&mytime);
                    cq.tm_buf[cq.idx] = (unsigned long long)(mytime.tv_sec) * 1000000 + (unsigned long long)(mytime.tv_usec);

				
					if(bio->bi_bdev != NULL) if(bio->bi_bdev->bd_super != NULL) if(bio->bi_bdev->bd_super->s_type != NULL)
						cq.tp_buf[cq.idx] = bio->bi_bdev->bd_super->s_type->name;
